...
namespace crm
{
    public partial class frm_pronostico_ventas_secundario : Form
    {
        ...
        private void btn_pronosticar_Click(object sender, EventArgs e)
        {
            ...
            //    }
            //    //MessageBox.Show(N.ToString());  VERSION CRMVWS
            ...
            if (cmb_clasificacion.Text == "Negocios")
            {
                ...
                // relleno de la primera fila, mes ante-pasado
                //columna1["nom_mes"] = nombre_mes.nombre_mes(Convert.ToInt32(uno) + 1);
                columna1["nom_mes"] = nombre_mes.nombre_mes(Convert.ToInt32(uno) + 3);
                columna1["x"] = uno;
                columna1["y"] = contador_negos_2ant_mes;
                dt_nego_ganado_hijo.Rows.Add(columna1);

                // relleno de la segunda fila, mes pasado
                columna["nom_mes"] = nombre_mes.nombre_mes(Convert.ToInt32(dos) + 3);
                columna["x"] = dos;
                columna["y"] = contador_negos_ant_mes;
                dt_nego_ganado_hijo.Rows.Add(columna);

                // relleno de la tercera fila, este mes
                columna3["nom_mes"] = nombre_mes.nombre_mes(Convert.ToInt32(tres) + 3);
                columna3["x"] = tres;
                columna3["y"] = contador_negos_este_mes;
                dt_nego_ganado_hijo.Rows.Add(columna3);
                ...
                if (rbt_sig_trimestre.Checked)
                {
                    ...
                    for (double i = cont_meses; cont_meses <= 6; cont_meses++)
                    {
                        ...
                        nueva_fila["nom_mes"] = nombre_mes.nombre_mes(Convert.ToInt32(cont_meses) + 3); //***********++ NOTA++******* SUMAR +2 EN MAYO +3 EN JUNIO
                        nueva_fila["x"] = cont_meses;
                        nueva_fila["y"] = y.ToString("N2");
                ...
                else if (rbt_sig_anio.Checked)
                {
                    ...
                    for (double i = cont_meses; cont_meses <= 9; cont_meses++) // CAMBIOS AQUI EL OTRO MES ES 9
                    {
                        y = a + b * (cont_meses);
        ...
        private void btn_promedio_moviles_Click(object sender, EventArgs e)
        {
            ...
            if (cmb_clasificacion_p_movil.Text == "Negocios")
            {
                ...
                //nueva_fila_moviles["nom_mes"] = nombre_mes.nombre_mes(5); // el siguiente mes hay que cambiarlo a 6 para que sea junio
                nueva_fila_moviles["nom_mes"] = nombre_mes.nombre_mes(7); // el siguiente mes hay que cambiarlo a 7 para que sea julio
                nueva_fila_moviles["x"] = Convert.ToInt32(tres) + 1;
                nueva_fila_moviles["y"] = divi.ToString("N2");
        ...
        {
            //frm_informe_estadistico_proyeccion formulario = new frm_informe_estadistico_proyeccion();
            //formulario.dgv_contenedor.DataSource = dt_negos_finalizados;
            
            //formulario.fecha_inicio = nombre_mes.nombre_mes(4);
            //formulario.fecha_fin = nombre_mes.nombre_mes(7);

            //if (cmb_clasificacion.Text == "Negocios" || cmb_clasificacion_p_movil.Text == "Negocios")
            //{
            //    int decisionNegos = 1;
            //    formulario.deci = decisionNegos;

            //}
            //else if (cmb_clasificacion.Text == "Ventas" || cmb_clasificacion_p_movil.Text == "Ventas")
            //{
            //    int decisionVentas = 2;
            //    formulario.deci = decisionVentas;
            
            //}
            
            //    else { MessageBox.Show("No ha seleccionado ninguna opcion","Advertencia",MessageBoxButtons.OK, MessageBoxIcon.Exclamation); }
            //formulario.Show();
        }
