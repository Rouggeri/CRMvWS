...
namespace proyectoUOne
{
    public partial class CotizacionVista : Form
    {
        ...
        {
            try { 
            ModificacionControl.Enabled = true;

            string CodigoC = this.dgv_CotizacionVista.CurrentRow.Cells[0].Value.ToString();
            string fechaInicioC = this.dgv_CotizacionVista.CurrentRow.Cells[1].Value.ToString();
            string fechaTerminaC = this.dgv_CotizacionVista.CurrentRow.Cells[2].Value.ToString();
                string total = this.dgv_CotizacionVista.CurrentRow.Cells[4].Value.ToString();
            string clienteC = this.dgv_CotizacionVista.CurrentRow.Cells[6].Value.ToString();
            string nombrecliente = CapaDatos.ClienteNombre(CodigoC);
            txt_codigoTemporal.Text = CodigoC.Trim();
            dtpFechaI.Text = fechaInicioC.Trim();
            dtpFechaT.Text = fechaTerminaC.Trim();
            txtCliente.Text = nombrecliente.Trim();
                txtTotal.Text = total;
        }
        catch{
                MessageBox.Show("Error al cargar");
